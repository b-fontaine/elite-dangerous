...
### Trouver un contact de colonisation
...
### Héritage des factions mineures (mécanique communautaire)
...
Frontier présente explicitement ce mécanisme comme un moyen de « propager les factions du BGS en dehors de la Bulle » (voir la section [Impact sur le BGS](#impact-sur-le-bgs)).

### Concurrence et « sniping » des revendications
...
### Le coût croissant des ports supplémentaires
...
## Choix de l'orientation économique
...
Formalisée officiellement dans *Trailblazers – Update 3* (v4.1.2.0), cette mécanique détermine comment les installations d'un système interagissent économiquement entre elles :
...
- Le Type-9 Heavy et l'Imperial Cutter sont, avec le Panther Clipper Mk2 (non chiffré ici faute de vérification), les seuls vaisseaux du jeu à disposer de **deux emplacements internes de classe 8**, ce qui explique leur écart de capacité avec le Type-8.
- Le Type-8, malgré sa capacité brute nettement inférieure, embarque un **Frame Shift Drive de classe 5 de série** (contre classe 6 pour le Type-9 et classe 7 pour l'Imperial Cutter) sur une coque bien plus légère (400 t contre 850 t et 1 100 t) : à gabarit de FSD comparable après ingénierie, il conserve donc une meilleure portée de saut par trajet, ce qui en fait un choix pertinent pour des relais longue distance (bridging, cf. [Concurrence et « sniping » des revendications](#concurrence-et--sniping--des-revendications)) plutôt que pour du tonnage brut.
...
### Méthode de calcul du nombre de rotations
...
En appliquant cette formule aux tonnages déjà cités dans ce guide (voir [Tonnage de commodités requis](#tonnage-de-commodités-requis)) avec les capacités « tout cargaison » du tableau ci-dessus :
...
**Point non vérifié dans cette session** : les caractéristiques propres du Fleet Carrier lui-même (capacité du réservoir de tritium, consommation de carburant par saut, portée de saut maximale, temps de recharge entre deux sauts, coût d'achat et d'entretien) n'ont pas pu être confirmées auprès d'une source externe accessible pendant la rédaction de cette section (recherche web saturée, plusieurs sites de référence inaccessibles) ; elles ne sont donc volontairement pas chiffrées ici plutôt que devinées. Consulter le panneau de gestion du Fleet Carrier en jeu, ou [Outils](16-outils.md) pour un calculateur à jour, pour ces valeurs.

## Interface du contact « System Colonisation »

### Écran d'accueil et carte de revendication
...
### Suivi des systèmes et des chantiers
...
### Abandon et démolition : deux mécaniques distinctes
...
- La répartition complète à l'origine d'un système (60/20/13/6 %, cf. [Héritage des factions mineures](#héritage-des-factions-mineures-mécanique-communautaire)) est communautaire dans le détail chiffré (confirmée dans le texte du mega-guide), mais s'appuie sur le principe officiel ci-dessus.
...
## Impact sur l'économie et le lore (Thargoïdes)
...
## Points d'incertitude à garder en tête
...
- Les caractéristiques propres du **Fleet Carrier** (réservoir de tritium, consommation par saut, portée de saut, temps de recharge, coût d'achat et d'entretien) ont été **recherchées et non trouvées** auprès d'une source externe accessible lors de la rédaction de la section [Logistique de transport](#logistique-de-transport-capacité-utile-et-fleet-carrier-comme-dépôt-avancé) ; elles ont volontairement été omises plutôt que devinées.
